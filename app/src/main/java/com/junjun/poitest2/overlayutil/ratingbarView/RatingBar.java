package com.junjun.poitest2.overlayutil.ratingbarView;
...
public class RatingBar extends FrameLayout implements View.OnClickListener {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs == null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                mFillDrawable = context.getDrawable(R.mipmap.fill);
                mEmptyDrawable = context.getDrawable(R.mipmap.empty);
            } else {
                mFillDrawable = context.getResources().getDrawable(R.mipmap.fill);
                mEmptyDrawable = context.getResources().getDrawable(R.mipmap.empty);
            }
        } else {
